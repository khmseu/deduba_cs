...
def main() -> int:
    parser = argparse.ArgumentParser(description="CI checks: Doxygen warnings and Platform.h include ordering")
    parser.add_argument('--doxygen-path', default=str(DOXYFILE), help='Path to Doxyfile')
    ...
    if not shutil.which('doxygen'):
        print('Doxygen is required but not found. Please install it: apt-get install doxygen', file=sys.stderr)
        return 1
    ...
    if failed_files:
        ...
        for f in failed_files:
            print(' - ' + f)
        print('Please ensure that the first include in native shim .cpp files is "Platform.h".')
        return 1

    print('Doxygen produced no warnings and Platform.h is the first include for all shim .cpp files.')
    return 0
