...
extern "C" DLL_EXPORT ValueT *windows_GetNamedSecurityInfoW(const wchar_t *path, bool include_sacl) {
    ...
    if (result != ERROR_SUCCESS) {
        ...
        if (include_sacl && result == ERROR_PRIVILEGE_NOT_HELD) {
            ...
            result = GetNamedSecurityInfoW(
                const_cast<wchar_t *>(path), SE_FILE_OBJECT, secInfo, nullptr, nullptr, nullptr, nullptr, &pSD);
        }
    ...
    if (!ConvertSecurityDescriptorToStringSecurityDescriptorW(pSD, SDDL_REVISION_1, secInfo, &sddlString, nullptr)) {
